...
class Cell(genericDatatype.Cell):
	...
	def editHTML(self):
		...
		code = '<div class="rowDisplay" style="float:left;" data-editKey="'+ editKey +'" data-rowID="'+ (row.id if row else '') +'" data-tableName="'+(table.name if table else '')+'" data-ajaxURL="'+self.column.ajaxURL+'">'
		code += str(cell)
		code += '</div>'
					...
					if (!selectNewRow){
						function selectNewRow(editKey){
							...
							dataplus.chooseRow(tableName, function(rowID){
								...
								jQuery.get($(".rowDisplay[data-editKey="+editKey+"]").attr("data-ajaxURL"), {requestType: 'rowText', rowID: rowID.toString()}, function(data){
